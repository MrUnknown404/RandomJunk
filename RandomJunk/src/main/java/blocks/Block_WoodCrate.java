...
public class Block_WoodCrate extends Block implements IMetaBlockName, ITileEntityProvider {
	
	public static final PropertyEnum TYPE = PropertyEnum.create("type", CrateWoodTypes.class);
	
	public Block_WoodCrate() {
		super(Material.WOOD);
		//
		setHardness(2.0f);
		setResistance(20.0f);
		...
		//Get Names
		setUnlocalizedName(Reference.RandomJunkBlocks.WOODCRATE.getUnlocalizedName());
		setRegistryName(Reference.RandomJunkBlocks.WOODCRATE.getRegistryName());
	}
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityWoodCrate();
	}
	
	@Override
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		TileEntityWoodCrate te = (TileEntityWoodCrate) world.getTileEntity(pos);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!worldIn.isRemote) {
			playerIn.openGui(RandomJunk.instance, GuiHandler.WOODCRATE, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
